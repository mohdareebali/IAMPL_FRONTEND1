// src/screens/ManagerResetPassword.js
...
function ManagerResetPassword() {
  ...
  // identifier (company_id or email) + optional otp passed from OTP screen
  ...
  useEffect(() => {
    if (!identifier) {
      showMsg("Missing identifier. Redirecting to Forgot Password.", "error");
      setTimeout(() => navigate("/manager-forgot"), 1200);
    }
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      // build payload: include otp only if provided
      const payload = { identifier, newPassword };
      if (passedOtp) payload.otp = passedOtp;

      // POST to manager reset endpoint
      const res = await axios.post("http://localhost:5000/api/manager/reset-password", payload);

      showMsg(res.data?.message || "Password reset successful", "success", 4000);

      // after a short delay, navigate to manager login
      setTimeout(() => {
      ...
      // attempt to craft useful error message from backend
      ...
      else if (status === 400) errMsg = "Bad request — please check the OTP and try again.";
      else if (status === 401) errMsg = "Unauthorized — invalid OTP or credentials.";
      else if (status === 404) errMsg = "Company not found.";

      showMsg(errMsg, "error", 7000);
    } finally {
      setLoading(false);
    }
  };
