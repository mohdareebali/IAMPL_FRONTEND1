...
// ---------------- Field 11: Supplier Code (robust) ----------------
// Try to find the vendor code directly in the full extracted text first.
// Use several patterns (pipe-style, label-style, or vendor followed soon by digits).
let vendorDigits = null;

// Pattern A: "vendor | 249067 |" (common in your sample)
vendorDigits = text.match(/vendor\s*\|\s*(\d{4,})/i);

// Pattern B: "Vendor Code: 249067" or "Vendor Code 249067" or "Vendor: 249067"
if (!vendorDigits) {
  vendorDigits = text.match(/vendor(?:\s*code)?\s*[:.\-]?\s*(\d{4,})/i);
}

// Pattern C: "Vendor ... 249067" (vendor followed by up to a few non-digit chars then digits)
if (!vendorDigits) {
  vendorDigits = text.match(/vendor[\s\|\-:.,]{1,20}(\d{4,})/i);
}

if (vendorDigits) {
  data.supplierCode = vendorDigits[1];
} else {
  // Fallback — try the previous matchField() approach but ignore "Vendor Contact" blocks
  let vendorMatch = matchField("Vendor Code") || matchField("Vendor");
  if (vendorMatch && !/contact/i.test(vendorMatch)) {
    const digitsOnly = vendorMatch.match(/\d{4,}/);
    data.supplierCode = digitsOnly ? digitsOnly[0] : "";
  } else {
    data.supplierCode = "";
  }
}
  let poMatch = matchField('P.O.no') || matchField('Purchase order numver') || matchField('Purchase order no');
if (poMatch) {
  // Keep only digits
  data.purchaseOrderNumber = poMatch.replace(/\D/g, '');
} else {
...
const handlePdfExport = () => {
    ...
    const pageMarginBottom = 15; // Set a safe bottom margin for all sections
    ...
    const row1Values = [partNumberText, formData.partName || "", formData.serialNumber || "", formData.fairIdentifier || ""];
    ...
    let maxRow1Height = Math.max(...row1Values.map((val, i) => calculateBoxHeight(val, row1Widths[i], minBoxHeight)));
    
    // Page break check for Row 1
    if (currentY + maxRow1Height > pageHeight - pageMarginBottom) {
        doc.addPage();
        currentY = startY;
    }
    ...
    // Row 2
    const row2Values = [formData.partRevisionLevel || "", formData.drawingNumber || "", formData.drawingRevisionLevel || "", formData.additionalChanges || ""];
    ...
    let maxRow2Height = Math.max(...row2Values.map((val, i) => calculateBoxHeight(val, row2Widths[i], minBoxHeight)));
    
    // Page break check for Row 2
    if (currentY + maxRow2Height > pageHeight - pageMarginBottom) {
        doc.addPage();
        currentY = startY;
    }
    ...
    // Row 3
    const row3Values = [formData.manufacturingProcessReference || "", formData.organizationName || ""];
    const row3Widths = [boxWidth / 2, boxWidth / 2];
    const row3Labels = ["9. MANUFACTURING PROCESS REFERENCE", "10. ORGANIZATION NAME"];
    let maxRow3Height = Math.max(...row3Values.map((val, i) => calculateBoxHeight(val, row3Widths[i], minBoxHeight)));
    
    // Page break check for Row 3
    if (currentY + maxRow3Height > pageHeight - pageMarginBottom) {
        doc.addPage();
        currentY = startY;
    }
    ...
    // Row 4
    const row4Values = [formData.supplierCode || "", formData.purchaseOrderNumber || ""];
    const row4Widths = [boxWidth / 2, boxWidth / 2];
    const row4Labels = ["11. SUPPLIER CODE", "12. PURCHASE ORDER NUMBER"];
    let maxRow4Height = Math.max(...row4Values.map((val, i) => calculateBoxHeight(val, row4Widths[i], minBoxHeight)));
    
    // Page break check for Row 4
    if (currentY + maxRow4Height > pageHeight - pageMarginBottom) {
        doc.addPage();
        currentY = startY;
    }
    ...
    // FAI section
    const faiSectionHeight = 40; // Approximate height for this section
    if (currentY + faiSectionHeight > pageHeight - pageMarginBottom) {
        doc.addPage();
        currentY = startY;
    }
    const faiSectionY = currentY;
    doc.setFontSize(8);
    ...
    doc.text("13.", marginX, faiSectionY + 4);
    doc.text("14. FAI Type:", faiTypeLabelX, faiSectionY + 4);
    
    doc.rect(marginX + checkboxXOffset, faiSectionY + 1, checkboxWidth, checkboxWidth);
    if (formData.detailFAI) { doc.setFontSize(6); doc.text("v", marginX + checkboxXOffset + checkboxCenterOffset, faiSectionY + 1 + checkboxCenterOffset + 1, { align: 'center' }); doc.setFontSize(8); }
    doc.text("Detail FAI", marginX + checkboxXOffset + checkboxWidth + 2, faiSectionY + 4);

    doc.rect(marginX + checkboxXOffset + 30, faiSectionY + 1, checkboxWidth, checkboxWidth);
    if (formData.assemblyFAI) { doc.setFontSize(6); doc.text("v", marginX + checkboxXOffset + 30 + checkboxCenterOffset, faiSectionY + 1 + checkboxCenterOffset + 1, { align: 'center' }); doc.setFontSize(8); }
    doc.text("Assembly FAI", marginX + checkboxXOffset + 30 + checkboxWidth + 2, faiSectionY + 4);
    
    doc.rect(faiTypeLabelX + faiTypeCheckboxOffset, faiSectionY + 1, checkboxWidth, checkboxWidth);
    if (formData.fullFAI) { doc.setFontSize(6); doc.text("v", faiTypeLabelX + faiTypeCheckboxOffset + checkboxCenterOffset, faiSectionY + 1 + checkboxCenterOffset + 1, { align: 'center' }); doc.setFontSize(8); }
    doc.text("Full FAI", faiTypeLabelX + faiTypeTextOffset, faiSectionY + 4);

    doc.rect(faiTypeLabelX + faiTypeCheckboxOffset + 30, faiSectionY + 1, checkboxWidth, checkboxWidth);
    if (formData.partialFAI) { doc.setFontSize(6); doc.text("v", faiTypeLabelX + faiTypeCheckboxOffset + 30 + checkboxCenterOffset, faiSectionY + 1 + checkboxCenterOffset + 1, { align: 'center' }); doc.setFontSize(8); }
    doc.text("Partial FAI", faiTypeLabelX + faiTypeTextOffset + 30, faiSectionY + 4);
    ...
    doc.text(reasonLines, faiTypeLabelX, faiSectionY + 12);
    
    let currentYForReason = faiSectionY + 12 + (reasonLines.length * 4);

    doc.text("AOG", faiTypeLabelX, currentYForReason + 4);
    doc.rect(faiTypeLabelX + 10, currentYForReason + 1, checkboxWidth, checkboxWidth);
    if (formData.aog) { doc.setFontSize(6); doc.text("v", faiTypeLabelX + 10 + checkboxCenterOffset, currentYForReason + 1 + checkboxCenterOffset + 1, { align: 'center' }); doc.setFontSize(8); }
    ...
    if (formData.faaApproved) { doc.setFontSize(6); doc.text("v", faiTypeLabelX + 55 + checkboxCenterOffset, currentYForReason + 1 + checkboxCenterOffset + 1, { align: 'center' }); doc.setFontSize(8); }

    currentY = currentYForReason + 10;
    ...
    // Check for page break before the table starts
    const tableHeightEstimate = (rows.length + 1) * 10; // A rough estimate
    if (currentY + tableHeightEstimate > pageHeight - pageMarginBottom) {
        doc.addPage();
        currentY = startY;
    }
    ...
    rows.forEach((row, i) => {
        ...
        // Page break check before drawing each row
        if (tableY + rowHeight > pageHeight - pageMarginBottom) {
            doc.addPage();
            tableY = startY; // Reset Y for new page
            // Re-draw headers on the new page
            doc.setFont("helvetica", "bold");
            let headerX = marginX;
            tableHeaders.forEach((header, idx) => {
                doc.rect(headerX, tableY, colWidths[idx], 7);
    ...
    // Row for Customer, Program, To Division
    currentY += 5;
    let rowCustHeights = [
    ...
    if (currentY + maxRowCustHeight > pageHeight - pageMarginBottom) {
        doc.addPage();
        currentY = startY;
    }
    ...
    currentY += 5;
    const nonConformanceHeight = 10;
    if (currentY + nonConformanceHeight > pageHeight - pageMarginBottom) {
        doc.addPage();
        currentY = startY;
    }
    ...
    if (formData.fairNonconformance === 'Yes') { doc.setFontSize(6); doc.text("v", nonconformanceCheckboxX + checkboxCenterOffset, currentY + 1 + checkboxCenterOffset + 1, { align: 'center' }); doc.setFontSize(8); }
    ...
    if (formData.fairNonconformance === 'No') { doc.setFontSize(6); doc.text("v", nonconformanceCheckboxX + 15 + checkboxCenterOffset, currentY + 1 + checkboxCenterOffset + 1, { align: 'center' }); doc.setFontSize(8); }
    currentY += box19Height;

    // Row 20-21
    let row20Heights = [calculateBoxHeight(formData.fairVerifiedBy || "", boxWidth / 2, minBoxHeight), calculateBoxHeight(formData.fairVerifiedDate || "", boxWidth / 2, minBoxHeight)];
    let maxRow20Height = Math.max(...row20Heights);
    if (currentY + maxRow20Height > pageHeight - pageMarginBottom) {
        doc.addPage();
        currentY = startY;
    }
    ...
    // Row 22-23
    let row22Heights = [calculateBoxHeight(formData.fairReviewedBy || "", boxWidth / 2, minBoxHeight), calculateBoxHeight(formData.fairReviewedDate || "", boxWidth / 2, minBoxHeight)];
    let maxRow22Height = Math.max(...row22Heights);
    if (currentY + maxRow22Height > pageHeight - pageMarginBottom) {
        doc.addPage();
        currentY = startY;
    }
    ...
    // Row 24-25
    let row24Heights = [calculateBoxHeight(formData.customerApproval || "", boxWidth / 2, minBoxHeight), calculateBoxHeight(formData.customerApprovalDate || "", boxWidth / 2, minBoxHeight)];
    let maxRow24Height = Math.max(...row24Heights);
    if (currentY + maxRow24Height > pageHeight - pageMarginBottom) {
        doc.addPage();
        currentY = startY;
    }
    ...
    if (currentY + commentsHeight + 10 > pageHeight - pageMarginBottom) {
        doc.addPage();
        currentY = startY;
    }
